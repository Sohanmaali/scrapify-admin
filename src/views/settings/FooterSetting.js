...
export default function FooterSetting() {
  ...
  const handleSubmit = async (e, api) => {
    ...
    const updatedata = {
      value: api === "contact-info" ? contactData : socialLinks,
    };

    try {
      const response = await new BasicProvider(
        `cms/setting/${api}`
      ).postRequest(updatedata);
  ...
  return (
    <>
      <SubHeader name="Footer Setting" isHideAddButton={true} />
      <CContainer>
            ...
            <CCard>
              ...
              <CCardBody>
                <CForm
                  ...
                  onSubmit={(e) => {
                    handleSubmit(e, "contact-info");
                  }}
                >
                  {contactFields.map((field, index) => (
                    <CRow key={index}>
                      <CCol>
                        <CFormLabel htmlFor={field.id}>
                          {field.label}
                        </CFormLabel>
                        {field.type === "textarea" ? (
                          <CFormTextarea
          ...
          <CCol md={6}>
            <CCard>
              ...
              <CCardBody>
                <CForm
                  onSubmit={(e) => {
                    handleSubmit(e, "social-media");
                  }}
                >
                  {socialMediaPlatforms.map((platform) => (
                    <div key={platform.name}>
                      <CFormLabel htmlFor={platform.name}>
                        {platform.label}
                      </CFormLabel>
                      <CFormInput
                        type="text"
